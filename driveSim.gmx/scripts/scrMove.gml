...
if (hyp != 0) {
    ...
    while (abs(totalDXPos) < abs(maxDXPos) && !place_meeting(tryXPos + 20 * xStep, tryYPos, objBlock)) {
        tryXPos += xStep;
        totalDXPos += xStep;
    }
    
    while (abs(totalDYPos) < abs(maxDYPos) && !place_meeting(tryXPos, tryYPos - 20 * yStep, objBlock)) {
        tryYPos -= yStep;
        totalDYPos -= yStep;
    }
